...
# --- 3. Navigation ---
st.set_page_config(page_title="Secretary ERP - V129", layout="wide")
choice = st.sidebar.radio("Navigation (V129)", ["📊 Dashboard", "🏢 Company Register", "⚙️ Group Management", "📤 Data Exchange"])
...
if choice == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        with tab1:
            ...
            if t4.button("✅ Select All"): st.session_state.sel_v129 = True; st.rerun()
            if t5.button("🧹 Clear All"): st.session_state.sel_v129 = False; st.rerun()
            ...
            if not df_display.empty:
                ...
                edit_df = st.data_editor(df_display, column_config=col_cfg, disabled=disabled_cols, use_container_width=True, key="dash_v129")
                
                if st.button("💾 Save Batch Edits", key="btn_save_grid_v129"):
                    try:
                        with engine.begin() as conn:
                ...
                if len(selected) > 0:
                    ...
                    with st.popover("🧨 BATCH DELETE"):
                        ...
                        conf_b = st.text_input("Type DELETE", key="batch_del_v129")
        ...
        with tab2:
            ...
            if not df_alerts.empty:
                ...
                if 'sel_alert_v129' not in st.session_state: st.session_state.sel_alert_v129 = False
                if ta3.button("✅ Select All", key="sel_all_alert"): st.session_state.sel_alert_v129 = True; st.rerun()
                if ta4.button("🧹 Clear All", key="clr_all_alert"): st.session_state.sel_alert_v129 = False; st.rerun()
                ...
                if not df_alerts_display.empty:
                    df_alerts_display.insert(0, "Select", st.session_state.sel_alert_v129)
        ...
        with tab3:
            ...
            if 'sel_inv_v129' not in st.session_state: st.session_state.sel_inv_v129 = False
            if ti6.button("✅ Select All", key="sel_all_inv"): st.session_state.sel_inv_v129 = True; st.rerun()
            if ti7.button("🧹 Clear All", key="clr_all_inv"): st.session_state.sel_inv_v129 = False; st.rerun()
            ...
            if not df_inv.empty:
                ...
                if not df_inv_display.empty:
                    df_inv_display.insert(0, "Select", st.session_state.sel_inv_v129)
...
# --- 6. Company Register ---
                    ...
                    for t in t_tasks:
                        ...
                        if t['type'] == 'BR':
                            ...
                            if is_bvi:
                                ...
                                if is_all_in:
                                    br_task = next((x for x in t_tasks if x['type'] == 'BR'), None)
                                    if br_task:
                                        afr_by_val = br_task['s'].get('br_paid_by', 'Firm')
                                        afr_dt_str = br_task['s'].get('br_date')
                                    else:
                                        afr_by_val = 'Firm'
                                        afr_dt_str = None
    ...
    if inc_place == "HK":
        ...
        with cc1: n2e = st.date_input("Effective Date (Appt)", value=to_date(d['n2e']), min_value=MIN_DATE, key="n2e_v129", format="YYYY/MM/DD")
        with cc2: n2f = st.date_input("Filing Date (ND2A)", value=to_date(d['n2f']), min_value=MIN_DATE, key="n2f_v129", format="YYYY/MM/DD")
        with cc3:
        ...
        with cc5: n4e = st.date_input("Effective Date (Resign)", value=to_date(d['n4e']), min_value=MIN_DATE, key="n4e_v129", format="YYYY/MM/DD")
        with cc6: n4f = st.date_input("Filing Date (ND4)", value=to_date(d['n4f']), min_value=MIN_DATE, key="n4f_v129", format="YYYY/MM/DD")
        with cc7:
            st.info("Statutory Period: 15 days")
        ...
        with c_nn1: nn6_e = st.date_input("Effective Date", value=to_date(d['nn6_e']), min_value=MIN_DATE, key="nn6_e_v129", format="YYYY/MM/DD")
        with c_nn2: nn6_f = st.date_input("Filing Date (NN6)", value=to_date(d['nn6_f']), min_value=MIN_DATE, key="nn6_f_v129", format="YYYY/MM/DD")
        with c_nn3:
            st.info("Statutory Period: 1 Month")
            if nn6_e:
                nn6_deadline = add_one_month(nn6_e)
                st.markdown(f"**Deadline: :red[{nn6_deadline.strftime('%Y/%m/%d')}]**")
        with c_nn4: nn6_d = st.checkbox("Downloaded", value=d['nn6_d'], key="nn6_d_v129")
    ...
    if mode in ["🆕 Add New", "📋 Copy Existing"]:
        if st.button("💾 Save To Cloud", key="btn_save_v129"):
            if missing: st.error(f"❌ Missing mandatory fields: {', '.join(missing)}")
                ...
                try:
                    pd.DataFrame([row_v129]).to_sql('companies', engine, if_exists='append', index=False)
        ...
        with u_col.popover("🆙 Update"):
            if st.button("Confirm Update (Sync Main & Branches)", key="btn_update_v129"):
                if missing: st.error(f"❌ Missing mandatory fields: {', '.join(missing)}")
                else:
                    try:
        ...
        with d_col.popover("🚨 DELETE"):
            st.error(f"Delete {target_name} and ALL its branches?"); conf_s = st.text_input("Type DELETE", key="single_del_v129")
            if st.button("Confirm Delete Record", disabled=(conf_s != "DELETE"), key="btn_del_single_v129"):
                df_all = df_all[df_all['name_en'] != target_name]
...
# --- 8. Data Exchange ---
    ...
    c2.download_button(label="📦 Export All", data=buf_e.getvalue(), file_name=f"Backup_{now_dx}.xlsx", key="btn_export_all_v129")
