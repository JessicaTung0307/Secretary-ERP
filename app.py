...
# --- 3. Navigation ---
st.set_page_config(page_title="Secretary ERP - V128", layout="wide")
choice = st.sidebar.radio("Navigation (V128)", ["📊 Dashboard", "🏢 Company Register", "⚙️ Group Management", "📤 Data Exchange"])
...
if choice == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        with tab1:
            ...
            if t4.button("✅ Select All"): st.session_state.sel_v128 = True; st.rerun()
            if t5.button("🧹 Clear All"): st.session_state.sel_v128 = False; st.rerun()
            ...
            if not df_display.empty:
                ...
                edit_df = st.data_editor(df_display, column_config=col_cfg, disabled=disabled_cols, use_container_width=True, key="dash_v128")
                
                if st.button("💾 Save Batch Edits", key="btn_save_grid_v128"):
                    try:
                        with engine.begin() as conn:
                ...
                if len(selected) > 0:
                    ...
                    with st.popover("🧨 BATCH DELETE"):
                        ...
                        conf_b = st.text_input("Type DELETE", key="batch_del_v128")
        ...
        with tab2:
            ...
            if not df_alerts.empty:
                ...
                if 'sel_alert_v128' not in st.session_state: st.session_state.sel_alert_v128 = False
                if ta3.button("✅ Select All", key="sel_all_alert"): st.session_state.sel_alert_v128 = True; st.rerun()
                if ta4.button("🧹 Clear All", key="clr_all_alert"): st.session_state.sel_alert_v128 = False; st.rerun()
                ...
                if not df_alerts_display.empty:
                    df_alerts_display.insert(0, "Select", st.session_state.sel_alert_v128)
        ...
        with tab3:
            ...
            if 'sel_inv_v128' not in st.session_state: st.session_state.sel_inv_v128 = False
            if ti6.button("✅ Select All", key="sel_all_inv"): st.session_state.sel_inv_v128 = True; st.rerun()
            if ti7.button("🧹 Clear All", key="clr_all_inv"): st.session_state.sel_inv_v128 = False; st.rerun()
            ...
            if not df_inv.empty:
                ...
                if not df_inv_display.empty:
                    df_inv_display.insert(0, "Select", st.session_state.sel_inv_v128)
...
# --- 6. Company Register ---
    ...
    if inc_place == "HK" or is_hk_reg or is_bvi:
        ...
        if base:
            ...
            process_years = list(range(2024, current_system_year + 5))
            ...
            for y in process_years:
                ...
                if incorp_year and y < incorp_year:
                    full_state[y_str] = {'br_paid_by': 'N/A', 'br_date': None, 'afr_fee_by': 'N/A', 'afr_fee_date': None, 'ar_date': None, 'ar_cr_status': 'N/A', 'es_fee_by': 'N/A', 'es_fee_date': None, 'es_date': None, 'es_status': 'N/A'}
                else:
                    ...
                    br_dt = to_date(y_data.get('br_date', y_data.get('fee_date')))
                    afr_dt = to_date(y_data.get('afr_fee_date'))
                    ar_dt = to_date(y_data.get('ar_date'))
                    es_fee_dt = to_date(y_data.get('es_fee_date'))
                    es_dt = to_date(y_data.get('es_date'))
                    ...
                    if is_bvi:
                        ...
                        ar_cr_final = get_stat(ar_dt, ar_dl, y == incorp_year)
                        es_stat_final = get_stat(es_dt, es_dl, y == incorp_year)
                    else:
                        ...
                        es_dl = None
                        ar_cr_final = ar_cr if ar_cr else ('Completed' if ar_dt else 'Pending')
                        es_stat_final = 'N/A'
                        afr_by = 'N/A'; es_by = 'N/A'
                        afr_dt = None; es_fee_dt = None; es_dt = None
                        
                    full_state[y_str] = {
                        'br_paid_by': br_by, 'br_date': br_dt.strftime('%Y-%m-%d') if br_dt else None,
                        'afr_fee_by': afr_by, 'afr_fee_date': afr_dt.strftime('%Y-%m-%d') if afr_dt else None,
                        'ar_date': ar_dt.strftime('%Y-%m-%d') if ar_dt else None, 'ar_cr_status': ar_cr_final,
                        'es_fee_by': es_by, 'es_fee_date': es_fee_dt.strftime('%Y-%m-%d') if es_fee_dt else None,
                        'es_date': es_dt.strftime('%Y-%m-%d') if es_dt else None, 'es_status': es_stat_final
                    }
                    
                    tasks.append({'type_order': 1, 'type': 'BR', 'fy': y, 'dy': br_dl.year if br_dl else y, 'dl': br_dl, 's': full_state[y_str]})
                    tasks.append({'type_order': 2, 'type': 'AR', 'fy': y, 'dy': ar_dl.year if ar_dl else y, 'dl': ar_dl, 's': full_state[y_str]})
                    if is_bvi: tasks.append({'type_order': 3, 'type': 'ES', 'fy': y, 'dy': es_dl.year if es_dl else y, 'dl': es_dl, 's': full_state[y_str]})
            ...
            for T in disp_tabs:
                ...
                t_tasks = sorted(t_tasks, key=lambda x: (x['type_order'], x['fy']))
                ...
                with st.expander(f"📌 Tasks Due in {T}", expanded=(T==current_system_year)):
                    for t in t_tasks:
                        ...
                        if t['type'] == 'BR':
                            ...
                            br_by_val = c1.selectbox(f"Paid By (FY {y})", pay_opts, index=pay_opts.index(s['br_paid_by']) if s['br_paid_by'] in pay_opts else 0, key=f"br_by_{T}_{y}")
                            if br_by_val == "N/A":
                                c2.text_input(f"Paid Date (FY {y})", value="N/A", disabled=True, key=f"br_dt_dis_{T}_{y}")
                                br_dt_val = None
                            else:
                                dt_obj = to_date(s['br_date'])
                                br_dt_val = c2.date_input(f"Paid Date (FY {y})", value=dt_obj, min_value=MIN_DATE, key=f"br_dt_{T}_{y}", format="YYYY/MM/DD")
                            stat = get_stat(br_dt_val, dl, False)
                            c3.text_input(f"Status (FY {y})", value=stat, disabled=True, key=f"br_st_dis_{T}_{y}")
                            s['br_paid_by'] = br_by_val
                            s['br_date'] = br_dt_val.strftime('%Y-%m-%d') if br_dt_val else None
                            ...
                            if is_bvi:
                                ...
                                if is_all_in:
                                    afr_by_val = full_state[str(y)].get('br_paid_by', 'Firm')
                                    c1.selectbox(f"AFR Fee By (FY {y})", [afr_by_val], index=0, disabled=True, key=f"afr_by_dis_{T}_{y}")
                                    afr_dt_str = full_state[str(y)].get('br_date')
                                    c2.text_input(f"AFR Fee Date (FY {y})", value=afr_dt_str if afr_dt_str else "N/A", disabled=True, key=f"afr_dt_dis2_{T}_{y}")
                                    afr_dt_val = to_date(afr_dt_str)
                                else:
                                    afr_by_val = c1.selectbox(f"AFR Fee By (FY {y})", pay_opts, index=pay_opts.index(s['afr_fee_by']) if s['afr_fee_by'] in pay_opts else 0, key=f"afr_by_{T}_{y}")
                                    if afr_by_val == "N/A":
                                        c2.text_input(f"AFR Fee Date (FY {y})", value="N/A", disabled=True, key=f"afr_dt_dis3_{T}_{y}")
                                        afr_dt_val = None
                                    else:
                                        afr_dt_val = c2.date_input(f"AFR Fee Date (FY {y})", value=to_date(s['afr_fee_date']), min_value=MIN_DATE, key=f"afr_dt_{T}_{y}", format="YYYY/MM/DD")
                                
                                if y == incorp_year:
                                    c3.text_input(f"AFR Filed Date (FY {y})", value="N/A (Exempt)", disabled=True, key=f"ar_dt_dis_x_{T}_{y}")
                                    ar_dt_val = None
                                else:
                                    ar_dt_val = c3.date_input(f"AFR Filed Date (FY {y})", value=to_date(s['ar_date']), min_value=MIN_DATE, key=f"ar_dt_{T}_{y}", format="YYYY/MM/DD")
                                ar_stat = get_stat(ar_dt_val, dl, y == incorp_year)
                                st.caption(f"**Status:** {ar_stat}")
                                
                                s['afr_fee_by'] = afr_by_val
                                s['afr_fee_date'] = afr_dt_val.strftime('%Y-%m-%d') if afr_dt_val else None
                                s['ar_date'] = ar_dt_val.strftime('%Y-%m-%d') if ar_dt_val else None
                                s['ar_cr_status'] = ar_stat
                            else:
                                if y == incorp_year or s['ar_cr_status'] == 'Exempt (Dormant)':
                                    c1.text_input(f"AR Filed Date (FY {y})", value="N/A (Exempt/Dormant)", disabled=True, key=f"ar_dt_dis_hk_{T}_{y}")
                                    ar_dt_val = None
                                else:
                                    ar_dt_val = c1.date_input(f"AR Filed Date (FY {y})", value=to_date(s['ar_date']), min_value=MIN_DATE, key=f"ar_dt_hk_{T}_{y}", format="YYYY/MM/DD")
                                
                                cr_idx = cr_opts.index(s['ar_cr_status']) if s['ar_cr_status'] in cr_opts else 0
                                ar_cr_val = c2.selectbox(f"AR CR Status (FY {y})", cr_opts, index=cr_idx, key=f"ar_cr_hk_{T}_{y}")
                                
                                s['ar_date'] = ar_dt_val.strftime('%Y-%m-%d') if ar_dt_val else None
                                s['ar_cr_status'] = ar_cr_val
                                s['afr_fee_by'] = 'N/A'
                                s['afr_fee_date'] = None
                            ...
                            c1, c2, c3 = st.columns(3)
                            es_by_val = c1.selectbox(f"ES Fee By (FY {y})", pay_opts, index=pay_opts.index(s['es_fee_by']) if s['es_fee_by'] in pay_opts else 0, key=f"es_by_{T}_{y}")
                            if es_by_val == "N/A":
                                c2.text_input(f"ES Fee Date (FY {y})", value="N/A", disabled=True, key=f"es_fee_dt_dis_{T}_{y}")
                                es_fee_dt_val = None
                            else:
                                es_fee_dt_val = c2.date_input(f"ES Fee Date (FY {y})", value=to_date(s['es_fee_date']), min_value=MIN_DATE, key=f"es_fee_dt_{T}_{y}", format="YYYY/MM/DD")
                                
                            if y == incorp_year:
                                c3.text_input(f"ES Filed Date (FY {y})", value="N/A (Exempt)", disabled=True, key=f"es_f_dt_dis_{T}_{y}")
                                es_dt_val = None
                            else:
                                es_dt_val = c3.date_input(f"ES Filed Date (FY {y})", value=to_date(s['es_date']), min_value=MIN_DATE, key=f"es_f_dt_{T}_{y}", format="YYYY/MM/DD")
                            es_stat = get_stat(es_dt_val, dl, y == incorp_year)
                            st.caption(f"**Status:** {es_stat}")
                            
                            s['es_fee_by'] = es_by_val
                            s['es_fee_date'] = es_fee_dt_val.strftime('%Y-%m-%d') if es_fee_dt_val else None
                            s['es_date'] = es_dt_val.strftime('%Y-%m-%d') if es_dt_val else None
                            s['es_status'] = es_stat
            
            updated_comp_json = full_state
    ...
    if inc_place == "HK":
        ...
        with cc1: n2e = st.date_input("Effective Date (Appt)", value=to_date(d['n2e']), min_value=MIN_DATE, key="n2e_v128", format="YYYY/MM/DD")
        with cc2: n2f = st.date_input("Filing Date (ND2A)", value=to_date(d['n2f']), min_value=MIN_DATE, key="n2f_v128", format="YYYY/MM/DD")
        with cc3:
        ...
        with cc5: n4e = st.date_input("Effective Date (Resign)", value=to_date(d['n4e']), min_value=MIN_DATE, key="n4e_v128", format="YYYY/MM/DD")
        with cc6: n4f = st.date_input("Filing Date (ND4)", value=to_date(d['n4f']), min_value=MIN_DATE, key="n4f_v128", format="YYYY/MM/DD")
        with cc7:
            st.info("Statutory Period: 15 days")
        ...
        with c_nn1: nn6_e = st.date_input("Effective Date", value=to_date(d['nn6_e']), min_value=MIN_DATE, key="nn6_e_v128", format="YYYY/MM/DD")
        with c_nn2: nn6_f = st.date_input("Filing Date (NN6)", value=to_date(d['nn6_f']), min_value=MIN_DATE, key="nn6_f_v128", format="YYYY/MM/DD")
        with c_nn3:
            st.info("Statutory Period: 1 Month")
            if nn6_e:
                nn6_deadline = add_one_month(nn6_e)
                st.markdown(f"**Deadline: :red[{nn6_deadline.strftime('%Y/%m/%d')}]**")
        with c_nn4: nn6_d = st.checkbox("Downloaded", value=d['nn6_d'], key="nn6_d_v128")
    ...
    if mode in ["🆕 Add New", "📋 Copy Existing"]:
        if st.button("💾 Save To Cloud", key="btn_save_v128"):
            if missing: st.error(f"❌ Missing mandatory fields: {', '.join(missing)}")
                ...
                try:
                    pd.DataFrame([row_v128]).to_sql('companies', engine, if_exists='append', index=False)
        ...
        with u_col.popover("🆙 Update"):
            if st.button("Confirm Update (Sync Main & Branches)", key="btn_update_v128"):
                if missing: st.error(f"❌ Missing mandatory fields: {', '.join(missing)}")
                else:
                    try:
        ...
        with d_col.popover("🚨 DELETE"):
            st.error(f"Delete {target_name} and ALL its branches?"); conf_s = st.text_input("Type DELETE", key="single_del_v128")
            if st.button("Confirm Delete Record", disabled=(conf_s != "DELETE"), key="btn_del_single_v128"):
                df_all = df_all[df_all['name_en'] != target_name]
...
# --- 8. Data Exchange ---
    ...
    c2.download_button(label="📦 Export All", data=buf_e.getvalue(), file_name=f"Backup_{now_dx}.xlsx", key="btn_export_all_v128")
